...
public class KinectManager : MonoBehaviour 
{
	...
	public bool useMultiSourceReader = true;
...
//	public bool useOwnThread = false; 
	...
	protected bool kinectInitialized = true; 
	...
	protected bool bLimitedUsers = true;
...
//		}
	...
	{
		if(dictUserIdToIndex.ContainsKey(firstID))
		{
			int index = dictUserIdToIndex[firstID];
	...
	{
		if (firstID == 0) return Vector3.zero;
		int index = dictUserIdToIndex[firstID];
		
		if(dictUserIdToIndex.ContainsKey(index))
		{
			Debug.Log("tamam1");
			//int index = dictUserIdToIndex[userId];
...
//						}
		...
		//Debug.Log(GetJointKinectPosition(2,KinectManager.Instance.GetJointIndex(KinectInterop.JointType.HandRight)));
	...
	public void RefreshAvatarUserIds()
	{
		for(int i = 0; i < avatarControllers.Count; i++)
		{
			...
			if(oldUserId != newUserId)
			{
				...
				{
					WarningSystem.Events.showWarningEvent.Invoke("NOT DETECTED PLAYER");
				}
