				...
				if(manager.IsJointTracked(userId, i))
				{
					...
					else 
					{
						...
						if (manager.IsJointTracked (userId, p)) 
						{
							...
							if (i == 19 | i == 15 | i == 13 |i == 17)
							{
								if (collider.transform.gameObject.GetComponent<Temas>() == null)
								{
									collider.transform.gameObject.AddComponent<Temas>();
									collider.size = new Vector2(0.25f, (posJoint - posParent).magnitude);
								}
							}
							else 
